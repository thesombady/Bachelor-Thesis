...
def plotmesh(Data):
	plt.imshow(Data, origin = 'lower', extent = [0,3 * 100 ,0,3 * 100])  # , interpolation='bilinear')
	plt.colorbar()
	plt.show()

path = os.path.join("/Users/andreasevensen/Documents/GitHub/Bachelor-Thesis", 'Data')
path1 = os.path.join(path, 'EulerAbove100_100.npy')
Data = parser(path1)
plotmesh(Data[0].reshape(3 * 100, -1).real)